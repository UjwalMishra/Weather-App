...
const fetchApi = async (target) => {
    try {
        const url = `http://api.weatherapi.com/v1/current.json?key=07350ac0f54941db81d165434241302&q=${target}`;
        const response = await fetch(url);
        const data = await response.json();
        console.log(data);

        const {
            current: { cloud, temp_c, wind_kph, humidity, condition: { icon, text } },
            location: {
                name, region, localtime
            }
        } = data;
